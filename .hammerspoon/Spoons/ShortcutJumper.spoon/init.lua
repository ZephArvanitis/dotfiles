--- === ShortcutJumper ===
---
--- Make it easy to jump where you want
...
obj.version = "0.1"
obj.author = "Zeph Arvanitis <zeph@rescale.com"
obj.homepage = "TODO"
...
function obj:setUp(platformMapping, shortcutsFile, platformShortcutsFile)
    -- construct shortcut mapping
    -- First, remove newlines in the editable file.
    os.execute("tr -d '\n' < " .. shortcutsFile .. " > ~/.hammerspoon/.shortcuts.json")
    os.execute("tr -d '\n' < " .. platformShortcutsFile .. " > ~/.hammerspoon/.platform-shortcuts.json")
    homedir = os.getenv("HOME")
    local shortcutChoices = {}
    filecontents = io.open(homedir .. "/.hammerspoon/.shortcuts.json")
    for _, shortcut in ipairs(hs.json.decode(io.open(homedir .. "/.hammerspoon/.shortcuts.json"):read())) do
    ...
    local platforms = platformMapping
    ...
    for _, shortcut in ipairs(hs.json.decode(io.open(".platform-shortcuts.json"):read())) do
        for platformName, baseurl in pairs(platforms) do
            ...
            if platformName == "LOC" or platformName == "IL5ST" or platformName == "IL5PR" then
    ...
    obj.shortcutChooser = nil
    obj.shortcutChooser = hs.chooser.new(function(choice)
        if not choice then focusLastFocused(); return end
        -- local query = string.lower(shortcutChooser:query())
        local full_query = obj.shortcutChooser:query()
    ...
    obj.shortcutChooser:queryChangedCallback(function()
        -- adapted from:
        ...
        local query = string.lower(obj.shortcutChooser:query())
        ...
        if query == '' then
            ...
            obj.shortcutChooser:choices(shortcutChoices)
        else
            local choices = {}
            ...
            obj.shortcutChooser:choices(choices)
        end
    end)


    obj.shortcutChooser:searchSubText(true)
    obj.shortcutChooser:choices(shortcutChoices)

    obj.shortcutChooser:rows(5)
    obj.shortcutChooser:bgDark(true)

end

--- ShortcutJumper:bindHotkeys(mapping)
--- Method
--- Binds hotkeys for ShortcutJumper
---
--- Parameters:
...
---
--- Returns:
---  * The ShortcutJumper object
function obj:bindHotkeys(mapping)
    local spec = {
        show = function() self.shortcutChooser:show() end
    }
    hs.spoons.bindHotkeysToSpec(spec, mapping)
    -- from Caffeine template
